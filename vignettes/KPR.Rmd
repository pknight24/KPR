...
genera, and the age of each subject. We will fit a kernel penalized regression model
with the microbial abundances as the predictors, and subject age as
the outcome.
...
denotes the squared entries of $\mathbf{D}$.
...
```{r}
kpr.out <- KPR(designMatrix = Z, Y = Y, H = H, Q = Q)
```
...
respectively. In this example, we are not using a matrix of
covariates, and so it is fine to leave that parameter missing from the
function call.
